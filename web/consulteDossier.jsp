...
<form action="<%out.print("Navigation?action=modifierDossier&idDossier=" + d.getId());%>" method="POST" class="form-horizontal">        
    ...
    <fieldset>
        ...
        <div class="form-group">
            <label for="login" class="col-sm-2 control-label">Pays</label>
            <div class="col-sm-3">
                <input type="text" name="login" id="login" class="form-control" value="<%out.print(d.getEtudiant().getPays());%>" required readonly>
            </div>
        </div>
    </fieldset>
    ...
    <fieldset>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-3">
                   <input type="text" name="date" id="login" class="form-control" value="<%SimpleDateFormat formater = new SimpleDateFormat("dd MMMM yyyy", Locale.FRANCE);
                    out.print(formater.format(d.getDate()));%>" required readonly>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-3"> 
                <select name="etat" <% if(c.getType().getValue() <= TypeCompte.responsable_administrative.getValue()){ %>disabled<%}%> class="form-control">
    ...
    <div class="form-group">
        <label for="login" class="col-sm-2 control-label">Formation </label>
        <div class="col-sm-3">
            <input type="text" class="form-control" value="<% out.print(d.getDemandeFormation().getIntitule()); %>" required readonly>
        </div>
    </div>

    <% try {
            EtudiantEtranger e = (EtudiantEtranger) d.getEtudiant();
    %>
    <fieldset>
        <div class="form-group">
            <label for="login" class="col-sm-2 control-label">Nationnalité</label>
            <div class="col-sm-3">
                <input type="text" name="login" id="login" class="form-control" value="<% out.print(TypeJustificatifEtranger.etranger); %>" required readonly>
            </div>
        </div>
        <div class="form-group">
            <label for="login" class="col-sm-2 control-label">Niveau</label>
            <div class="col-sm-3">
                <input type="text" name="login" id="login" class="form-control" value="<% out.print(e.getNiveau()); %>" required readonly>
            </div>
        </div>
        <div class="form-group">
            <label for="login" class="col-sm-2 control-label">Avis</label>
            <div class="col-sm-3">
                <textarea type="text" name="login" id="login" class="form-control" readonly><% out.print(e.getAvis()); %></textarea>
            </div>
        </div>
    </fieldset>
    <%} catch (Exception e) {
    ...
    <fieldset>
        ...
        <div class="form-group" <% if (c.getType() != TypeCompte.admin) {
                if (c.getType() != TypeCompte.responsable_commission || !(d.getEtat() == TypeEtatDossier.en_attente_commission || d.getEtat() == TypeEtatDossier.navette)) {%>hidden<%}
                    }%>>
    ...
    <fieldset>
        ...
        <div class="form-group" <% if (c.getType() != TypeCompte.admin) {
                if (c.getType() == TypeCompte.responsable_commission || !(d.getEtat() == TypeEtatDossier.en_transfert_vers_directeur || d.getEtat() == TypeEtatDossier.navette_directeur)) {%>hidden<%}
                    }%>>
            <label class="radio-inline col-md-offset-2">
        ...
        <div class="form-group" <% if (c.getType() == TypeCompte.admin || (c.getType() == TypeCompte.directeur_pole && (d.getEtat() == TypeEtatDossier.en_transfert_vers_directeur || d.getEtat() == TypeEtatDossier.navette_directeur))) {%>hidden<%}%>>
...
<div class="row">
    ...
    <div class="col-md-2 col-md-offset-3">
        ...
        <%} else if (d.getAvisCommission()==TypeAvisDossier.défavorable) {%>
        ...
        %>
        <a class="btn btn-info" href="Navigation?action=genererLettre&idDossier=<%out.print(d.getId()); %>&typeLettre=4">Generer lettre d'audition</a>
        <%
        } else if (d.getEtat() == TypeEtatDossier.retour_vers_secretariat){
        %>
        <a class="btn btn-info" href="Navigation?action=cloture&id=<%out.print(d.getId()); %>">Cloturer le dossier</a>
        <%
        }
        %>
    </div>
